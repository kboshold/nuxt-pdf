...
interface LetterBProps extends Omit<PDFWrapperProps, 'foldMark'> {
  foldMark?: boolean
}

const props = withDefaults(defineProps<LetterBProps>(), {
  size: 'A4',
  marginLeft: 25,
  marginRight: 20,
  marginBottom: 20,
...
const wrapperProps = computed<PDFWrapperProps>(() => ({
  ...props,
  foldMark: props.foldMark ? 'letterB' : false,
}))
</script>
...
<template>
  <PDFWrapper v-bind="wrapperProps">
    <div class="w-full h-full z-20">
      <!-- Letterhead: 45mm (DIN 676 Form B) -->
      <div class="relative" :style="`height: 45mm; margin-top: ${-props.marginTop}mm; margin-left: ${-props.marginLeft}mm; margin-right: ${-props.marginRight}mm`">
        <slot name="letterhead" />
        <div class="absolute w-[75mm] h-[30mm] bottom-0" :style="`right: ${props.marginRight - 10}mm;`">
          <slot name="logo" />
        </div>
      </div>
